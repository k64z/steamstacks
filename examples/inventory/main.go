// Command inventory prints the authenticated account's TF2 backpack
// (appid 440, context 2) using the steamcommunity inventory endpoint.
//
// Run the login example first to create steam_session_web.json, or set
// STEAM_USERNAME / STEAM_PASSWORD / STEAM_SHARED_SECRET to log in fresh.
package main
...
func main() {
	...
	if err != nil {
		log.Fatalf("get inventory: %v", err)
	}

	fmt.Printf("%d items\n", len(items))
	for _, item := range items {
		fmt.Printf("  [%s] %s (tradable: %v)\n", item.AssetID, item.Name, item.Tradable)
	}
}
...
func loadOrLogin(ctx context.Context) *steamsession.Session {
	...
	// Web cookies authenticate requests to steamcommunity.com and
	// store.steampowered.com; they must be populated once per process.
